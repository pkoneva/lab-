...
class Variable : public Expression {
	char c;
public:
	Variable() { throw "Ââåäèòå ïåðåìåííóþ"; }
	Variable(char x) { c = x; }
...
void Stack::composite() {
	...
	if (zn == -1) return;
	Expression* e2 = st_expr.e_Pop();// âûòàëêèâàåòñÿ â îáðàòíîì ïîðÿäêå
...
Expression* Stack::create() {
	...
	while (fin >> c) {
		...
		if ((num = Find_Number(c)) != -10000000000) {
			...
			en = new Number(num);
			st_expr.e_Push(en);
			int long_file = fin.tellg(); //ñ÷èòàí ëèøíèé ñèìâîë, íåîáõîäèìî ïåðåäâèíóòü òåêóùóþ ïîçèöèþ íà 1 áàéò íàçàä
